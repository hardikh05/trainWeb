...
app.post("/register", async(req, res) =>{
    try {
        ...
        const newUser = await pool.query(
            "INSERT INTO users1 (FirstName, LastName, Email, ContactNo, Password) VALUES ($1, $2, $3, $4, $5)",
...
app.post("/getTrains", async(req, res) =>{
    ...
    try {
        ...
        for(let i=0;i<trains.length;i++){
            let currentTrain = await pool.query(
    ...
    try {
        ...
        for(let i=0;i<req.route.length;i++){
            const newRoute = await pool.query("INSERT INTO Routes (TrainID, CurrentStation, RemainingSeats, TimefromStart, CurrentDate, RouteID) VALUES($1, $2, $3, $4, $5, $6);",
                [newTrain.trainid, req.routes[i].station.toLowerCase(), req.totalseats, req.route[i].timeFromStart, d1, maxRouteId.max+1]
            );
        }
        for(let i=0;i<req.routes.length;i++){
            const newRoute = await pool.query("INSERT INTO Routes (TrainID, CurrentStation, RemainingSeats, TimefromStart, CurrentDate, routeID) VALUES($1, $2, $3, $4, $5, $6);",
                [newTrain.trainid, req.route[i].station.toLowerCase(), req.totalseats, req.route[i].timeFromStart, d2, maxRouteId.max+2]
            );
        }
